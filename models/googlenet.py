...
class SubGoogLeNet(nn.Module):
    def __init__(self):
        super(SubGoogLeNet, self).__init__()
        self.pre_layers = nn.Sequential(
            nn.Conv2d(3, 192, kernel_size=3, padding=1),
            nn.BatchNorm2d(192),
            nn.ReLU(True),
    ...
    def sub_backward(self, targets):
        ...
        back_var = self.avgpool(self._inter_var[-2])
        back_var = back_var.view(back_var.size(0), -1)
        back_var = self.linear(back_var)
        back_var.backward(self._inter_var[-1].grad)

        back_var = self.a5(self._inter_var[-3])
        back_var = self.b5(back_var)
        back_var.backward(self._inter_var[-2].grad)

        back_var = self.d4(self._inter_var[-4])
        back_var = self.e4(back_var)
        back_var = self.maxpool(back_var)
        back_var.backward(self._inter_var[-3].grad)

        back_var = self.a4(self._inter_var[-5])
        back_var = self.b4(back_var)
        back_var = self.c4(back_var)
        back_var.backward(self._inter_var[-4].grad)

        back_var = self.pre_layers(self._inter_var[-6])
        back_var = self.a3(back_var)
        back_var = self.b3(back_var)
        back_var = self.maxpool(back_var)
